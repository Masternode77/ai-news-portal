...
const netapp = {
  ...
  articleText: Array.from({ length: 4 }, () => 'NetApp announced a set of data management updates for Red Hat OpenShift to improve backup predictability, disaster recovery, and operational scalability across on-premises and cloud-based virtualized environments. The release focuses on OpenShift Virtualization deployments, where growing VM counts and larger datasets can make traditional full-disk backup approaches harder to operate. Platform teams can watch restore speed, DR failover validation, and cross-environment management as AI workloads move from pilot to production.').join(' '),
