...
| Tests, build, content gate | PASS | Build-backed hermetic suite: 687 total, 687 passed, 0 failed, 0 skipped, with the pre-existing tracked diff preserved byte-for-byte. The final adversarial 129-test admin/auth/state/publish set passed three consecutive runs (387/387). Astro check reports 0 errors, 0 warnings, and 0 hints; the 62-page build, public/content/image/admin/performance gates, browser admin E2E, and dependency audit pass. |
| Preview and rendered QA | PASS | Exact preview public/retired/admin route contracts and desktop/mobile/article rendering pass with no broken images, placeholders, browser errors, or overflow. A deployment-bound adversarial receipt records three 10/10 hostile HTTP passes and 31/31 unique homepage image hashes. |
...
| Current upstream content | BLOCKED (operation) | Read-only audit of `origin/main` at `f345f679` finds 27 source-only candidates; advisory title-only triage is 2 core, 6 adjacent, and 19 archive-shaped. Pipeline `5.6.2` rejects old checkpoints, missing editorial/Image2 providers before state access, zero-success extraction/QA batches, provider fallback, non-distinct or missing image variants, and publication bundles without all reconciled image paths. Provider-backed re-ingestion has not completed because the linked Vercel project currently has no environment variables. |
| Upstream integration | BLOCKED (operation) | `npm run audit:integration` runs native merge semantics in an isolated temporary object database and reports 8 conflicts across the retired dashboard artifact, 4 generated image variants, and 3 generated JSON projections. The tracked receipt is `docs/upstream-integration-preflight.md`; raw merging is rejected. |
...
or promote that deployment. A fresh same-viewport comparison differs on 75.1509% of pixels:
