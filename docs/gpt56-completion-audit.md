...
| Tests, build, content gate | PASS | Full suite: 622 total, 621 passed, 0 failed, 1 intentional skip. Astro check reports 0 errors, 0 warnings, and 0 hints; the 59-page build, public/content/image/admin/performance gates, browser admin E2E, and dependency audit pass. |
...
| Current upstream content | BLOCKED (operation) | Read-only audit of `origin/main` at `b3544f5a` finds 25 source-only candidates; advisory title-only triage is 2 core, 6 adjacent, and 17 archive-shaped, but guarded canonical re-ingestion has not been executed. |
...
- Deployment: `dpl_J5jbRixDCBLoqEvRqN4gmZKKVvWs`, target `preview`, status `READY`.
...
- Captures: `artifacts/preview-f735cc40/` contains exact-preview desktop/mobile screenshots,
...
latest observed production deployment is `dpl_Bt7BbS4jdFCAN7aMcw3zizDNFJBs`; current `origin/main`
was separately observed at `b3544f5a34b48ff8bf89877e18513122ee3cf29b`. This branch did not create
