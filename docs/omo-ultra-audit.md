...
- Current public homepage cards after product-fit and source-rights gates: 0. Retained JSON records are not treated as reader-visible cards.
...
- Feed registry: `config/sourceRegistry.yml` contains 27 registered sources; `activeRegistryFeeds()` currently returns 0 authorized feeds.
...
- Existing env constants include image, OpenRouter, Supabase, and pipeline settings in `scripts/lib/constants.mjs` (9964 bytes).
...
- Retained records marked article-page-published with non-blog_engine_v4 generation version: 0; examples: none found. Current public detail eligibility separately requires product fit, source rights, and final publication integrity.
- Retained source-only/direct-link items: 921.
