...
- Current public homepage cards after product-fit and source-rights gates: 33. Retained JSON records are not treated as reader-visible cards.
...
- Primary public data: `src/data/latest-news.json` (42 records).
- Archive data: `src/data/archived-news.json` (967 records).
...
- Feed registry: `config/sourceRegistry.yml` contains 76 registered sources; `activeRegistryFeeds()` currently returns 22 authorized feeds.
...
- Retained source-only/direct-link items: 1004.
