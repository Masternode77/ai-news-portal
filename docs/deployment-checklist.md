...
Relevance scores come from the article's own words (title, snippet, body, source name and URL). The registry `defaultCategory` is a routing hint, never scoring input — folding the label into the text once made every item from an `AI Infrastructure (GPU/Neocloud)` source read as a full memo before a word of it was fetched. When adding a source, verify a few of its items with `Feed Probe` (`feed-probe.yml`, input `urls`) first: it prints HTTP status, content type, and item counts from a GitHub-hosted runner, which is the only place feed URLs can be checked from restricted environments.

Pool composition: `selectPoolItems` keeps only items newer than `POOL_MAX_AGE_DAYS` (10) when anything fresh exists, caps each source at `MAX_ITEMS_PER_SOURCE_IN_POOL` (6), and orders by fetch-time relevance with recency breaking ties. A feed that exposes months of history (NRC ships 165 items) otherwise fills the 30-slot pool with stale notices. The one-item-per-source reservation only applies to items that are at least signal-card relevant, so a feed whose best item is off-beat (a hydro licence notice, an enforcement action, a proclamation) does not displace on-beat items; that is what makes it safe to register broad government feeds. Procedural docket notices from docket sources (Federal Register, FERC, regulations.gov, govinfo: hydro relicensing, EIS availability, pipeline blanket authorizations, exempt wholesale generator and information-collection notices, recognised by their formulaic titles or a "Take notice that" opener) are also capped below the signal-card threshold and archived by the public lane router unless their source evidence (title, extracted body or feed snippet, never the generated summary) carries an unambiguous compute or large-load term (a bare "training", "inference" or "accelerator" does not count), because their extracted text alone saturates the grid dimension (run #3199 put a hydro SEIS notice on the homepage at 0.615); articles from other sources are not subject to that guard. Feeds from the same publication share a source name (all Federal Register feeds are `Federal Register`, all arXiv feeds are `arXiv`) so the cap applies per publication. When an agency site blocks the runner (ferc.gov behind Cloudflare, bis.gov without a feed), the Federal Register agency feed `https://www.federalregister.gov/api/v1/documents.rss?conditions%5Bagencies%5D%5B%5D=<agency-slug>` carries the same public-domain record; the brackets must be URL-encoded or curl rejects the URL. The 2026-09-10 source expansion and its probe evidence are in `docs/source-expansion-2026-09.md`. The wire publishes at most `ITEMS_PER_RUN` items per KST slot; a verification run outside the schedule needs `force_wire=true` on the `Update News` dispatch (sets `PIPELINE_FORCE_SLOT=1`), otherwise a slot that already ran picks nothing.

Extraction artifacts: every public gate (`currentSourceTextAuthorization`, `publicProductFitResult`, detail-route eligibility) validates `extraction_artifact` through `validateExtractionArtifact`, which requires `extraction_qa.public_publishable === true`, an empty `block_reasons` list, and a sentence-completion score of at least 0.92. `enrichContent` therefore builds the artifact with `analyzeSourceExtractionFailClosed` (the same policy the tier router and homepage filter use), and the pipeline rebuilds older artifacts that predate the flag from the stored source text on every run. An artifact built from raw quality metrics alone is never publishable, which is how the public surface froze on the pinned inventory between August 12 and September 3 while the pipeline kept committing records.
...
EIA restoration step: `restore-eia-public-inventory.mjs` re-fetches the fifteen EIA articles on every run and rebuilds their records from the live text. When EIA serves a truncated or error page (a 146-character "article" failed run #3182 on 2026-09-05), `resolveSourceTexts` retries once and then reuses the text stored in the committed inventory, but only when that record's extraction artifact validates (hash match, same source URL, ≥500 characters); the log line `[restore-eia] <id>: live extraction too short (n); reusing verified text from the committed inventory` records the substitution. With no verified stored text the step still fails closed, so a first-time restoration cannot be faked from a bad fetch.
