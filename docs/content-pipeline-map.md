...
- `parseFeedItem()` also attaches a preliminary infrastructure relevance score from `scripts/lib/relevance-classifier.mjs` using RSS title/snippet/body fields. A registry row with `text_scope: abstract` (arXiv, whose authorized text is CC0 metadata only) reaches the item as `source_text_scope`, and the classifier caps such items at the signal-card lane because an abstract cannot support the 4,500-character local memo contract. `authorizedTextFallbackPool()` re-stamps that scope from the registry and reclassifies cached or legacy pool records (`hydrateSourceTextScope()`), so the cap also holds when a run falls back to `news-pool.json` or uses `PIPELINE_USE_EXISTING_POOL`. `columnCandidateRecords()` re-stamps the scope on surface and archive records before column selection, and `selectColumnStory()` in `scripts/lib/authored-column-engine.mjs` refuses an abstract-only record as a column's primary source, recognising legacy records through their registry row as well as the stamped field. The autonomous desk keeps the scope through `scanSourceItems()` in `scripts/lib/global-source-scan.mjs`, and `selectEditorialSignals()` caps a cluster anchored on an abstract-only source at `Watchlist Signal`. The public lane router (`routeStrictInfrastructureRelevance()`) routes such a source to the adjacent lane whatever its score, so `applyPublicRouting()`, `canGenerateFullArticle()` and the regeneration scripts (which re-stamp the scope from the registry before routing) cannot recreate a long-form page from an abstract; `abstractOnlyTextScope()` in `scripts/lib/source-registry.mjs` is the shared detector.
...
- Procedural docket notices from government docket sources (Federal Register, FERC, regulations.gov, govinfo: hydro relicensing, EIS availability, pipeline blanket authorizations, exempt wholesale generator and information-collection notices, recognised by their formulaic titles or FERC's "Take notice that" opener) are capped at 0.44 (`procedural_regulatory_docket_without_compute_context`) unless the text carries an unambiguous compute or large-load term (AI, GPU, data center, colocation, hyperscale, semiconductor, HPC, large load, co-location, server racks or farms; a bare "training", "inference", "accelerator" or "servers" does not count) in the source evidence (title, extracted body or feed snippet, source metadata; never the generated summary or insight); `routeStrictInfrastructureRelevance()` archives them on the same predicate (`proceduralDocketWithoutComputeContext()`), whatever score a record already carries. Articles from other sources are never touched by this guard, even when they mention hydroelectric generation or pipelines. `refreshCachedRelevance()` in `fetch-feeds.mjs` (fallback pool, column candidates, autonomous scan, regenerators) re-stamps the registry text scope and demotes a cached record that the guard catches, so a stale stored classification cannot outlive the guard. `rollingCandidates()` in `curate.mjs` drops definitively archived items (`definitivelyArchived()`: a docket notice, a hard archive topic, or an item already extracted and still archive-only) before the curation model or the deterministic ranker sees them; the autonomous scan drops docket notices before clustering and `selectEditorialSignals()` archives any cluster still anchored on one. The guard reads `source_evidence_text`/`cleaned_source_text` first and the wire body fields only before extraction, never generated prose. The public product-fit projection carries the registry id and source URL (not as scoring text) so `isPublicProductFit()` fails a docket notice on every public surface, and `buildHomepageFeed()` never re-opens such an archive route through a stored tier.
