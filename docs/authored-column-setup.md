...
**Josh Inn**(운영자의 필명)이 하루 최대 3편, 가장 의미 있는 소식 하나를 골라 일관된 시각의 영문 에세이
...
큐레이션 모델은 `CURATION_MODEL` 시크릿으로 정하며 기본값은 OpenAI GPT-5.6 Sol(OpenRouter id `openai/gpt-5.6-sol`)입니다. 모델 id가 카탈로그에서 사라지면(400/404) `OPENROUTER_MODEL`로 한 번 재시도하고, 그래도 실패하면 결정론 랭커로 넘어갑니다.
