...
Loading occurs during scheduled refresh, not on page read. Empty data says no verified observations; stale data keeps its observation date and clear stale label; errors never turn into zero values. Korean pages set the document language to ko. The weekly digest is an on-site reading surface with RSS; do not add email enrollment or sender setup controls. Capacity year/state filters update visible totals and expose empty and reset states while retaining the full no-JavaScript table. Never expose API keys, internal pipeline IDs or budget errors in reader flows.

### Content, constraints and open questions

Keep concise source-derived copy and explicit observation vs forecast vs queue distinctions. Reuse static builds and existing libraries; data refresh is independent of the news pipeline. Acceptance: desktop/mobile rendering, no broken links, truthful empty/stale/failure states, and source/license checks. The monthly OpenRouter alert budget is $30. Artwork is registered from the active Codex session with no OpenAI API key. Email service/sender connection and publisher outreach were removed at the owner’s request.
